...
#             extended = extended[None,:]
        padded = np.pad(extended.astype(np.float64), ((0,0,),(0,max_list_size-extended.shape[1],),), 'constant', constant_values=np.nan)
...
def stack_padded_clicks(clicks, max_list_size):
    ...
    for qid in range(len(clicks)):
        padded = np.pad(clicks[qid].astype(np.float64), ((0,0,),(0,max_list_size-clicks[qid].shape[1],),),mode= 'constant', constant_values=np.nan)
